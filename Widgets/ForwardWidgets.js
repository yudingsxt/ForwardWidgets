    ...
    // --- 平台筛选模块（新增排序功能）---
    {
        ...
        params: [
            ...
            },
            {
                ...
                enumOptions: [
                    { title: "默认排序", value: "" },
                    { title: "流行度最高", value: "popularity.desc" },
                    { title: "投票数最多", value: "vote_count.desc" },
  ...
  if (videoIds.length === 0) {
      ...
      for (const itemId of itemElementIds) {
          try {
              ...
              console.error("IMDB 解析错误:", parseError, "HTML Item ID:", itemId);
          }
      }
...
async function loadBangumiRankings(params = {}) {
    ...
    try {
        ...
        console.error("加载 Bangumi 排行榜页面失败:", error);
...
async function loadBangumiBrowser(params = {}) {
    ...
    const url = basePath + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");

    console.log("请求 Bangumi 浏览页面:", url);
    ...
    try {
        ...
        if (listItems.length === 0) {
            const nextLink = Widget.dom.selectFirst(docId, ".page_inner > .p_cur + a");
            if (nextLink < 0) {
                console.log("Bangumi 浏览: 已到达末尾或无结果");
            } else {
